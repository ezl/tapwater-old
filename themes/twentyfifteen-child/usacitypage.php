...
if($query->have_posts()):// do not display if only one city in country, because that city  == current city
    ...
    ?>
    <h3>Check tap water safety for other states in <a href="<?php echo site_url('/tap-water-safety-in-').$country_slug; ?>"><?php echo $country_name; ?></a></h3>
    <ul class="country-list">
    <?php
    while($query->have_posts()):
        $query->the_post();
        if(get_post_type() == 'page' && get_post_field( 'post_name', get_the_ID()) != 'tap-water-safety-in-united-states-of-america')://only show pages - states
